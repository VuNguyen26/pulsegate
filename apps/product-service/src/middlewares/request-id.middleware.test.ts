...
describe("generateRequestId", () => {
  ...
  it("should generate a new request id when x-request-id is too long", () => {
    const suppliedRequestId = "a".repeat(129);
    expect(generateRequestId(createMockRequest({
      "x-request-id": suppliedRequestId,
    }))).not.toBe(suppliedRequestId);
  });

  it("should generate a new request id when x-request-id contains unsupported characters", () => {
    const suppliedRequestId = "request id with spaces";
    expect(generateRequestId(createMockRequest({
      "x-request-id": suppliedRequestId,
    }))).not.toBe(suppliedRequestId);
  });
